...
namespace AthleticsCompetition.repository
{
    class UsersRepo : IUsers
    {
        ...
        {
            log.InfoFormat("Entering findUser with value {0}", username);
            IDbConnection con = DBUtils.getConnection(props);
            ...
            using (var comm = con.CreateCommand())
            {
                ...
                using (var dataR = comm.ExecuteReader())
                {
                    if (dataR.Read())
                    {
                        ...
                        User user = new User(firstname1, lastname1, username, password);
                        user.Id = idV;
                        log.InfoFormat("Exiting findUser with value {0}", user);
                        return user;
                    }
                }
            }
            log.InfoFormat("Exiting findUser with value {0}", null);
            return null;
        }
