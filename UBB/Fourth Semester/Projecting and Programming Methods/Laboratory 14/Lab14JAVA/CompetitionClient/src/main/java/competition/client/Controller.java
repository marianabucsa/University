...
public class Controller implements INotificationSubscriber {
    @FXML
    ...
    public void setService(ServerInterface service, NotificationReceiver receiver) {
    ...
    @Override
    public void notificationReceived(Notification notification) throws RemoteException {
        try {
            System.out.println("Ctrl notificationReceived ... " + notification.getType());
            SwingUtilities.invokeLater(() -> {
                if (notification.getType() == NotificationType.ADDED_ENROLLMENT) {
                    try {
                        for (int i = 0; i< enrollmentChildren.size(); i++) {
                            EnrollmentDTO en = (EnrollmentDTO) enrollmentChildren.get(i);
                            if(Objects.equals(en.getRoundName(), notification.getText())) {
                                enrollmentChildren.remove(i);
                                break;
                            }
                        }
                        enrollmentChildren.add(new EnrollmentDTO(notification.getText(), service.getNrChildrenForRound(notification.getText())));
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    enrollmentsTable.setItems(enrollmentChildren);
