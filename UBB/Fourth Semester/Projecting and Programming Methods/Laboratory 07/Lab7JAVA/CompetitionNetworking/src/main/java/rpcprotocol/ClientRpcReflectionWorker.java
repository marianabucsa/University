...
public class ClientRpcReflectionWorker implements Runnable, ObserverInterface {
    ...
    private Response handleSAVE_ENROLLMENT(Request request) {
        ...
        try {
            ...
            return new Response.Builder().type(ResponseType.GET_CHILDREN_BY_AGEGROUP).data(enrollment).build();
        } catch (Exception ae) {
            return new Response.Builder().type(ResponseType.ERROR).data(ae.getMessage()).build();
        }
    }
